...
\begin{tikzpicture}[x=10cm,y=10cm]
    ...
    \addplot3[
        domain=-28:10, % One loop corresponds to one full cycle (2*pi)
    ...
    % Line segment
    \addplot3[
        domain=-100:-28, % One loop corresponds to one full cycle (2*pi)
        samples=50, % Increase the number of samples for a smoother curve
        variable=t,
        samples y = 0
    ]({t/5}, {cos(-10*28)}, {sin(-8*28) + (t/800) + 28/800});
...
\begin{multicols}{2}
    \fbox{\includegraphics[scale = 0.6]{Ea_gyak_1/2.png}}
    \columnbreak
